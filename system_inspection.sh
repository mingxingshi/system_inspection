...
report_One_Min_Load=""    #1分钟内每个逻辑CPU的平均负载
report_Five_Min_Load=""    #1分钟内每个逻辑CPU的平均负载
report_Fifteen_Min_Load=""    #1分钟内每个逻辑CPU的平均负载
...
function getLoadAvg(){
    ...
    echo "####################### CPU平均负载检查 ##########################"
    Virt_CPUs=$(grep "processor" /proc/cpuinfo | wc -l)
    One_Min_Load=$(echo $(uptime | awk -F "average:" '{print $2}' | awk -F "," '{print $1}') $Virt_CPUs | awk '{print $1/$2*100"%"}')
    Five_Min_Load=$(echo $(uptime | awk -F "average:" '{print $2}' | awk -F "," '{print $2}') $Virt_CPUs | awk '{print $1/$2*100"%"}')
    Fifteen_Min_Load=$(echo $(uptime | awk -F "average:" '{print $2}' | awk -F "," '{print $3}') $Virt_CPUs | awk '{print $1/$2*100"%"}')
    echo "1分钟内每个逻辑CPU的平均负载:$One_Min_Load"
    echo "5分钟内每个逻辑CPU的平均负载:$Five_Min_Load"
    echo "15分钟内每个逻辑CPU的平均负载:$Fifteen_Min_Load"
    #报表信息
    report_One_Min_Load=$One_Min_Load    #1分钟内每个逻辑CPU的平均负载
    report_Five_Min_Load=$Five_Min_Load  #5分钟内每个逻辑CPU的平均负载
    report_Fifteen_Min_Load=$Fifteen_Min_Load     #15分钟内每个逻辑CPU的平均负载
}
...
function uploadHostDailyCheckReport(){
    json="{
        ...
        \"Arch\":\"$report_Arch\",
        \"One_Min_Load\":\"$report_One_Min_Load\",
        \"Five_Min_Load\":\"$report_Five_Min_Load\",
        \"Fifteen_Min_Load\":\"$report_Fifteen_Min_Load\",
